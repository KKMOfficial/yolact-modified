...
# fixing seed for reprodecuability
# fix do we need the background class really? there is no background class specified inside the dataset
# model modification ability : we can change the layers, order of the layers of the backbones, also include different backbones
# model backbones are configured to receive RGB images
# model protonets are also configurable inside the data/config.py class:line 685/ yolact_base_config dictionary contains all the required information
# todo correct last layer bias settings
# todo monitor evaluation metrics
# evalution metrics are corrrently monitord by default are : 
# [ 16]     470 || B: 0.896 | C: 0.637 | M: 0.374 | S: 0.035 | T: 1.942 || ETA: 20 days, 4:33:43 || timer: 4.138
# todo one batch over fit
# todo verify dcreasing loss
# todo prediction dynamics
# Loss Key:
...
#  - I: Mask IoU loss
#  see yolact/layers/modules/multibox_loss.py
# todo see how the above losses are being calculated
overfit_stage        = False
# model optimizer is SGD, must convert it to ADAM with learning rate 3e-4
# there is a learning rate decay mechanism
reqularization_stage = False
# data augmentation information is available inside the config dictionary in data/config.py
tuning_stage         = False
# endregion
...
# region Update training parameters from the config if necessary
def replace(name):
...
class NetLoss(nn.Module):
    ...
    def forward(self, images, targets, masks, num_crowds, cur_index=0, log=False):
        ...
        if log:
          ...
          for x in preds['segm']:
            writer.add_images('output/masks_segmentations', torchvision.utils.make_grid(x.reshape((x.shape[0],1,x.shape[1],x.shape[2])))[None,:,:,:], global_step=cur_index)
...
def train():
    ...
    if pipeline_check_stage : torch.manual_seed(7)
    yolact_net = Yolact()

    if pipeline_check_stage : print(f"Network Structure\n{yolact_net}")
    ...
    if pipeline_check_stage : print(f"Criterion Informatin\n{criterion}")
    ...
    if pipeline_check_stage : print(f"Dataloader Informaton\n{data_loader}")
    ...
    if pipeline_check_stage : print(f"Number Of Epochs={num_epochs}")
    print()
